...
async function post(event,action){
  if(event.httpMethod!=='POST')return S.response(405,{ok:false,error:'Method not allowed.'});const b=S.parse(event),db=S.client(),id=S.text(b.id,80);await S.verifyTurnstile(event,b.turnstile_token);
  ...
  if(action==='update'){const next=S.validatePost({...row.data,...b},{partial:false});const out=await db.from('community_posts').update({...next,status:'pending',approved_at:null}).eq('id',id);if(out.error)throw out.error;return S.response(200,{ok:true,status:'pending'})}
  ...
  const out=await db.from('community_posts').update({status:'deleted',cleanup_after:new Date(Date.now()+7*86400000).toISOString()}).eq('id',id);if(out.error)throw out.error;return S.response(200,{ok:true,status:'deleted'});
}
module.exports={post};
