...
function edgeErrorText(e:any){
  if(!e) return '알 수 없는 서버 오류';
...
async function cleanup(region = 'dallas') {
  // V63: 일반 수집 기사는 14일 보관 후 자동 삭제합니다.
  // 원문 발행일(source_published_at)을 우선 기준으로 사용하고, 발행일이 없을 때만 수집일을 사용합니다.
  // 관리자가 '보관'한 기사는 기간과 관계없이 유지합니다.
  const retentionDays = 14;
  const cutoffMs = Date.now() - retentionDays * 24 * 60 * 60 * 1000;
  const { data: rows, error } = await admin.from('newsroom_items')
    .select('id,collected_at,source_published_at,event_data,status,original_title,ai_title')
    .eq('region', region)
    .limit(5000);
  ...
  for (const row of rows || []) {
    ...
    const basisRaw = row.source_published_at || row.collected_at || null;
    const basis = basisRaw ? new Date(basisRaw).getTime() : 0;
    if (basis && basis < cutoffMs) ids.push(row.id);
  }
  if (!ids.length) return { ok: true, cleaned: 0, retention_days: retentionDays, cutoff: new Date(cutoffMs).toISOString() };
  let cleaned = 0;
  ...
  return { ok: true, cleaned, retention_days: retentionDays, cutoff: new Date(cutoffMs).toISOString() };
}
...
function practicalLifeLine(item:any, topic:any) {
  ...
  const defaults:any = {
    ...
    culture:'생활 한 줄: 문화·예술 행사는 일정과 입장 조건이 바뀔 수 있으니 방문 전 최신 안내를 확인하세요.',
...
async function publishOne(region='dallas', force=false) {
  ...
  const { data: recentPosts } = await admin.from('posts')
    .select('subtype,created_at,title')
    .eq('region',region).eq('type','life')
    .order('created_at',{ascending:false}).limit(7);
  const recentSubtypes = new Set((recentPosts || []).map((x:any)=>String(x.subtype||'')).filter(Boolean));
  const eligible = (rows || []).filter((x:any)=>{
  ...
  const ranked = eligible.map((x:any)=>{
    const topic = lifeTopicForItem(x);
    const diversityBonus = topic && !recentSubtypes.has(topic.subtype) ? 1000 : 0;
    const freshness = Math.max(0, 200 - Math.floor((Date.now()-new Date(x.source_published_at||x.collected_at||0).getTime())/3600000));
    return {x,score:diversityBonus+Number(x.priority_score||0)+freshness};
  }).sort((a:any,b:any)=>b.score-a.score);
  const candidate = ranked[0]?.x || null;
...
async function automationStatus(region = 'dallas') {
  const today = dateKeyInDallas();
  const dayKeys = Array.from({length:7}, (_,i) => {
    const d = new Date(); d.setUTCDate(d.getUTCDate()-i); return dateKeyInDallas(d);
  });
  const since = new Date(Date.now() - 8 * 86400000).toISOString();
  const [{data:runs,error:re},{data:items,error:ie},{data:posts,error:pe}] = await Promise.all([
    admin.from('newsroom_runs').select('*').eq('region',region).gte('started_at',since).order('started_at',{ascending:false}).limit(300),
    admin.from('newsroom_items').select('id,status,collected_at,source_published_at,ai_title,ai_summary,ai_content,error_message,updated_at,event_data').eq('region',region).gte('collected_at',since).order('collected_at',{ascending:false}).limit(2000),
    admin.from('posts').select('id,title,created_at,type,region').eq('region',region).eq('type','life').gte('created_at',since).order('created_at',{ascending:false}).limit(200),
  ]);
  if (re) throw re; if (ie) throw ie;
  const safePosts = pe ? [] : (posts || []);
  const todayRuns=(runs||[]).filter((r:any)=>dateKeyInDallas(r.started_at)===today);
  const todayItems=(items||[]).filter((x:any)=>dateKeyInDallas(x.collected_at||x.source_published_at)===today);
  const todayPosts=safePosts.filter((x:any)=>dateKeyInDallas(x.created_at)===today && !String(x.title||'').startsWith('[테스트]'));
  const generated=todayItems.filter((x:any)=>Boolean(x.ai_title||x.ai_summary||x.ai_content)||['classified','review','published'].includes(String(x.status||'').toLowerCase()));
  const review=todayItems.filter((x:any)=>['collected','classified','review','hold','pending'].includes(String(x.status||'').toLowerCase()));
  const failedRuns=todayRuns.filter((r:any)=>String(r.status||'').toLowerCase()==='failed');
  const itemErrors=todayItems.filter((x:any)=>['error','failed'].includes(String(x.status||'').toLowerCase())||x.error_message);
  const latest=todayRuns[0]||null;
  const collectRuns=todayRuns.filter((r:any)=>/lane:|provider:|auto run/i.test(String(r.note||'')) || ['scheduled','cron','manual'].includes(String(r.trigger_type||'').toLowerCase()));
  const collectFailed=collectRuns.some((r:any)=>String(r.status||'').toLowerCase()==='failed');
  const collectRunning=collectRuns.some((r:any)=>String(r.status||'').toLowerCase()==='running');
  const stageCollect={status:collectFailed?'failed':collectRunning?'running':collectRuns.length?'success':'not_started',found:collectRuns.reduce((n:number,r:any)=>n+Number(r.found||0),0),inserted:collectRuns.reduce((n:number,r:any)=>n+Number(r.inserted||0),0),skipped:collectRuns.reduce((n:number,r:any)=>n+Number(r.skipped||0),0),latest:collectRuns[0]||null};
  const stageAnalyze={status:generated.length?'success':todayItems.length?'not_started':'not_started',found:todayItems.length,inserted:generated.length,skipped:todayItems.filter((x:any)=>String(x.status||'')==='excluded').length,latest:generated[0]?{started_at:generated[0].updated_at||generated[0].collected_at}:null};
  const stagePublish={status:todayPosts.length?'success':'not_started',found:generated.length,inserted:todayPosts.length,skipped:0,latest:todayPosts[0]?{started_at:todayPosts[0].created_at}:null};
  let overall='not_started';
  if (failedRuns.length) overall='failed';
  else if (collectRunning) overall='running';
  else if (stageCollect.status==='success' && stageAnalyze.status==='success' && stagePublish.status==='success') overall='complete';
  else if (stageCollect.status==='success' || todayItems.length || generated.length || todayPosts.length) overall='partial';
  const timeline:any[] = todayRuns.slice(0,20).map((r:any)=>({stage:'collect',status:r.status,started_at:r.started_at,finished_at:r.finished_at,found:r.found||0,inserted:r.inserted||0,skipped:r.skipped||0,note:r.note||'',error_message:r.error_message||''}));
  if (generated.length) timeline.push({stage:'analyze',status:'success',started_at:generated[0].updated_at||generated[0].collected_at,inserted:generated.length,skipped:todayItems.filter((x:any)=>String(x.status||'')==='excluded').length,note:'오늘 AI 분류/기사 생성 데이터 기준'});
  if (todayPosts.length) timeline.push({stage:'publish',status:'success',started_at:todayPosts[0].created_at,inserted:todayPosts.length,skipped:0,note:'오늘 posts 게시 기록 기준'});
  timeline.sort((a:any,b:any)=>new Date(b.started_at||0).getTime()-new Date(a.started_at||0).getTime());
  const errors:any[]=[...failedRuns.map((r:any)=>({stage:'collect',time:r.started_at,message:r.error_message||r.note||'수집 작업 실패'})),...itemErrors.map((x:any)=>({stage:'item',time:x.updated_at||x.collected_at,message:x.error_message||'기사 처리 오류'}))].slice(0,20);
  const history=dayKeys.map((key:string)=>{
    const di=(items||[]).filter((x:any)=>dateKeyInDallas(x.collected_at||x.source_published_at)===key);
    const dr=(runs||[]).filter((r:any)=>dateKeyInDallas(r.started_at)===key);
    const dp=safePosts.filter((x:any)=>dateKeyInDallas(x.created_at)===key&&!String(x.title||'').startsWith('[테스트]'));
    return {date_key:key,collected:di.length,generated:di.filter((x:any)=>Boolean(x.ai_title||x.ai_summary||x.ai_content)||['classified','review','published'].includes(String(x.status||'').toLowerCase())).length,published:dp.length,failed:dr.filter((r:any)=>String(r.status||'').toLowerCase()==='failed').length,success:dr.filter((r:any)=>String(r.status||'').toLowerCase()==='success').length};
  });
  return {ok:true,version:VERSION,overall,latest,counts:{collected:todayItems.length,generated:generated.length,published:todayPosts.length,review:review.length,errors:errors.length},stages:{collect:stageCollect,analyze:stageAnalyze,publish:stagePublish},timeline,errors,history,generated_at:new Date().toISOString()};
}


// ===== V64 Smart Weekly Flyer Edge actions =====
function flyerNum(v:any){
  if(v===null||v===undefined||v==='') return null;
  // Never turn 2/$5 into 25, or packaging text into a price.
  const text=String(v).trim();
  if(!/^\$?(?:\d+)(?:\.\d{1,2})?$/.test(text)) return null;
  const n=Number(text.replace(/^\$/,''));
  return Number.isFinite(n)&&n>=0?n:null;
}
function flyerDiscount(regular:any,sale:any){
  const r=flyerNum(regular), s=flyerNum(sale);
  if(!(r>0) || s===null || s<0 || s>=r) return null;
  return Math.max(0,Math.min(99,Math.round((1-s/r)*100)));
}
function flyerTitleFromBusiness(name=''){
  return String(name||'Weekly Market').trim().slice(0,80) || 'Weekly Market';
}
async function smartFlyerBusinessName(businessId:any){
  if(!businessId) return '';
  const tries=['id,name_ko,name_en,name','id,name_ko,name_en','id,name_ko,name','id,name_ko','id,name'];
  for(const cols of tries){
    const {data,error}=await admin.from('businesses').select(cols).eq('id',businessId).maybeSingle();
    if(!error && data) return String((data as any).name_ko||(data as any).name||(data as any).name_en||'').trim();
  }
  return '';
}
async function listWeeklyFlyers(body:any){
  const region=String(body.region||'dallas').toLowerCase();
  let q=admin.from('weekly_flyers')
    .select('*,weekly_flyer_items(*)')
    .eq('region',region)
    .order('created_at',{ascending:false});
  if(body.business_id) q=q.eq('business_id',String(body.business_id));
  if(body.id) q=q.eq('id',Number(body.id));
  const {data,error}=await q.limit(Math.min(200,Math.max(1,Number(body.limit)||100)));
  if(error) throw error;
  return {ok:true,version:VERSION,flyers:data||[]};
}
async function setWeeklyFlyerStatus(body:any){
  const id=Number(body.id||0);
  if(!id) throw new Error('전단 ID가 없습니다.');
  const patch:any={updated_at:new Date().toISOString()};
  if(body.status!==undefined) patch.status=String(body.status);
  if(body.show_on_home!==undefined) patch.show_on_home=Boolean(body.show_on_home);
  if(String(body.status||'').toLowerCase()==='archived' || String(body.status||'').toLowerCase()==='draft'){
    patch.show_on_home=false;
  }
  const {data,error}=await admin.from('weekly_flyers').update(patch).eq('id',id).select().maybeSingle();
  if(error) throw error;
  if(!data) throw new Error('전단을 찾지 못했습니다.');
  return {ok:true,version:VERSION,flyer:data};
}
async function deleteWeeklyFlyer(body:any){
  const id=Number(body.id||0);
  if(!id) throw new Error('전단 ID가 없습니다.');
  const {error:itemError}=await admin.from('weekly_flyer_items').delete().eq('flyer_id',id);
  if(itemError) throw itemError;
  const {data,error}=await admin.from('weekly_flyers').delete().eq('id',id).select('id');
  if(error) throw error;
  return {ok:true,version:VERSION,deleted:data?.length||0};
}
async function updateWeeklyFlyerItems(body:any){
  ...
  for(const item of items){
    ...
    if(!id) continue;
    const regular=flyerNum(item.regular_price);
    const sale=flyerNum(item.sale_price);
    const patch:any={
      product_name:String(item.product_name||'').trim().slice(0,180),
      regular_price:regular,
      sale_price:sale,
      unit_text:String(item.unit_text||'').trim().slice(0,80)||null,
      discount_percent:flyerDiscount(regular,sale),
      is_featured:Boolean(item.is_featured),
      updated_at:new Date().toISOString(),
    };
    if(!patch.product_name) continue;
    const {error}=await admin.from('weekly_flyer_items').update(patch).eq('id',id).eq('flyer_id',flyerId);
    if(error) throw error;
    updated++;
  }
  return {ok:true,version:VERSION,updated};
}
async function activateWeeklyFlyer(body:any){
  const id=Number(body.id||0);
  if(!id) throw new Error('전단 ID가 없습니다.');
  const {data:flyer,error:fe}=await admin.from('weekly_flyers')
    .select('*').eq('id',id).maybeSingle();
  if(fe) throw fe;
  if(!flyer) throw new Error('전단을 찾지 못했습니다.');

  const {count,error:ce}=await admin.from('weekly_flyer_items')
  ...
  let oldQ=admin.from('weekly_flyers').update({
  ...
  const {error:oe}=await oldQ;
  if(oe) throw oe;

  const {data,error}=await admin.from('weekly_flyers').update({
    status:'active',show_on_home:body.show_on_home!==false,updated_at:new Date().toISOString()
  }).eq('id',id).select().maybeSingle();
  if(error) throw error;
  return {ok:true,version:VERSION,flyer:data,item_count:count};
}


function normalizeFlyerSourceBox(v:any){
  if(!v || typeof v!=='object') return null;
  let x=Number(v.x), y=Number(v.y), width=Number(v.width), height=Number(v.height);
  // tolerate percentage (0..100) values from model
  if([x,y,width,height].some(n=>!Number.isFinite(n))) return null;
  if(Math.max(x,y,width,height)>1.5){
  ...
  // Keep the detected block intact; crop must not expand into neighboring prices.
  return {x:Number(x.toFixed(6)),y:Number(y.toFixed(6)),
    width:Number(width.toFixed(6)),height:Number(height.toFixed(6))};
}


// V269: parse a complete printed price expression, never a bag of digits.
function flyerPriceBlock(value:any,unit:any){
  const text=String(value??'').trim();
  const unitText=String(unit||'').trim();
  const bundle=text.match(/^(\d+)\s*\/\s*\$(\d+(?:\.\d{1,2})?)$/);
  if(bundle){
    if(Number(bundle[1])<1)return {price:null,unit:unitText||null};
    return {price:flyerNum(bundle[2]),unit:[`${bundle[1]}개 묶음 (${text})`,unitText].filter(Boolean).join(' · ')};
  }
  const single=text.match(/^\$?(\d+(?:\.\d{1,2})?)(?:\s*(?:\/|per\s+)?\s*(lb|lbs|ea|each|box|pack|pk|case))?$/i);
  if(!single)return {price:null,unit:unitText||null};
  const canonical=(v:string)=>v.toLowerCase().replace(/^lbs$/,'lb').replace(/^each$/,'ea').replace(/^pk$/,'pack');
  const statedUnit=unitText.match(/\b(lb|lbs|ea|each|box|pack|pk|case)\b/i)?.[1];
  if(single[2]&&statedUnit&&canonical(single[2])!==canonical(statedUnit))
    return {price:null,unit:unitText};
  return {price:flyerNum(single[1]),unit:unitText||single[2]||null};
}
const FLYER_PRICE_BLOCK_RULES=`
- Read product_name, sale_price, regular_price, unit_text and source_box from ONE product block. Never borrow an adjacent price.
- Add price_text containing the exact printed sale price expression (e.g. "$1.99/lb", "$12.99 EA", "2/$5"). Use null if unreadable or absent; never infer a price from another product or typical retail prices.
- For 2/$5 store sale_price=5 (the total for two), price_text="2/$5" and preserve the required quantity in unit_text. Never store 25 or silently divide into a unit price.
- Preserve printed lb/EA, pack quantity, box/case basis and membership/coupon/limit conditions in unit_text. A box or pack price is not a per-item price. For conditional prices use the exact visible amount with its membership/quantity conditions, never an unconditional price.
- regular_price must be null unless a separate regular price for the SAME quantity and product is printed. Add regular_price_text with that exact amount or null.
- source_box must enclose the same product name, product photo, price and unit; do not extend into a neighboring product. If these cannot be matched confidently, omit the uncertain product. Keep scanning other readable blocks.
`;

function normalizeSmartFlyerItems(raw:any[]){
  const rows=(Array.isArray(raw)?raw:[]).slice(0,30).map((x:any,index:number)=>{
    const regular=flyerNum('regular_price_text' in x?x.regular_price_text:x.regular_price);
    const block=flyerPriceBlock('price_text' in x?x.price_text:x.sale_price,x.unit_text);
    const declared=flyerNum(x.sale_price);
    const sale=declared!==null&&block.price!==null&&declared!==block.price?null:block.price;
    return {
      product_name:String(x.product_name||'').trim().slice(0,180),
      regular_price:regular,
      sale_price:sale,
      unit_text:String(block.unit||'').trim().slice(0,80)||null,
      discount_percent:flyerDiscount(regular,sale),
      is_featured:Boolean(x.is_featured) || index<2,
      ai_score:Math.max(0,Math.min(100,Number(x.ai_score)||0)),
      source_box:normalizeFlyerSourceBox(x.source_box),
      sort_order:index+1,
      created_at:new Date().toISOString(),
      updated_at:new Date().toISOString(),
    };
  }).filter((x:any)=>x.product_name && x.sale_price!==null && x.source_box);

  const seen=new Set<string>();
  return rows.filter((x:any)=>{
    const key=`${String(x.product_name).toLowerCase().replace(/\s+/g,' ').trim()}|${Number(x.sale_price).toFixed(2)}`;
    if(seen.has(key)) return false;
    seen.add(key);
    return true;
  });
}
function standardSmartFlyerTitle(businessName:string,businessId:string){
  const name=String(businessName||businessId||'Market').trim();
  return `${name} Weekly Sale`.slice(0,180);
}

async function analyzeWeeklyFlyer(body:any){
  const region=String(body.region||'dallas').toLowerCase();
  const businessId=String(body.business_id||'').trim();
  const imageUrl=String(body.image_url||'').trim();
  if(!businessId) throw new Error('업소 ID가 없습니다.');
  if(!/^https?:\/\//i.test(imageUrl)) throw new Error('전단 이미지 URL이 올바르지 않습니다.');

  const businessName=await smartFlyerBusinessName(businessId);
  const primary:any=await openai({
    model:env('NEWSROOM_OPENAI_MODEL')||'gpt-5-mini',
    input:[{
      role:'user',
      content:[
        {
          type:'input_text',
          text:`Analyze this supermarket weekly flyer image for DalTownMap.
Store/business: ${businessName||businessId}
Extract ALL clearly readable sale product blocks across the ENTIRE flyer, not just the first or largest promotion.
Return ONLY JSON:
{
 "items":[
   {
     "product_name":"product name, Korean if clearly identifiable otherwise concise original name",
     "regular_price":null,
     "sale_price":0.00,
     "unit_text":"lb/ea/pack/etc or null",
     "is_featured":false,
     "ai_score":0,
     "source_box":{"x":0.0,"y":0.0,"width":0.0,"height":0.0}
   }
 ]
}
Rules:
- scan the full image from top-left to bottom-right and inspect every visible product-price tile.
${FLYER_PRICE_BLOCK_RULES}
- sale_price must be numeric when a price is visible.
- regular_price is null unless a separate regular/original price is clearly printed.
- return up to 30 distinct products.
- choose up to 5 strongest deals as is_featured=true.
- ai_score 0-100 based on prominence/clarity/value.
- source_box is REQUIRED and uses normalized 0..1 coordinates: x=left, y=top, width, height.
- include the product photo and printed price in the box where practical; avoid neighboring products.
- do not use seasonal slogans such as Back to School, Labor Day, Grand Opening, Sale of the Week, etc. as a title; title is managed separately by DalTownMap.
- no commentary outside JSON.`
  ...
  let items=normalizeSmartFlyerItems(primary?.items||[]);
  let analysisPasses=1;
  let rescueUsed=false;

  // V69: 1차 인식이 3개 이하이면 같은 이미지를 더 강한 전면 스캔 지시로 자동 재분석합니다.
  if(items.length<=3){
    const rescue:any=await openai({
      model:env('NEWSROOM_OPENAI_MODEL')||'gpt-5-mini',
      input:[{
        role:'user',
        content:[
          {
            type:'input_text',
            text:`SECOND-PASS RECOVERY for a supermarket flyer. The first pass found only ${items.length} usable product(s), which is probably incomplete.
Store/business: ${businessName||businessId}

Perform a deliberate exhaustive visual scan of the FULL flyer:
1. divide the page mentally into a 4 x 4 grid;
2. inspect every grid cell;
3. identify every distinct product block with a clearly readable sale price;
4. return product name, price, unit and a normalized bounding rectangle for each block.

Return ONLY:
{"items":[{"product_name":"","regular_price":null,"sale_price":0.00,"unit_text":null,"is_featured":false,"ai_score":0,"source_box":{"x":0.0,"y":0.0,"width":0.0,"height":0.0}}]}

Requirements:
${FLYER_PRICE_BLOCK_RULES}
- target 8-30 distinct items when the flyer visibly contains that many;
- do NOT stop after the headline/featured deal;
- include small product blocks as long as name and price are readable;
- source_box must be normalized 0..1 and tightly surround that product block;
- omit only unreadable/uncertain products;
- no title and no commentary.`
          },
          {type:'input_image',image_url:imageUrl,detail:'high'}
        ]
      }]
    ...
    const recovered=normalizeSmartFlyerItems(rescue?.items||[]);
    analysisPasses=2;
    rescueUsed=true;

    // Prefer the more complete pass, but merge unique products if both contain useful items.
    const merged=[...recovered,...items];
    const seen=new Set<string>();
    items=merged.filter((x:any)=>{
      const key=`${String(x.product_name).toLowerCase().replace(/\s+/g,' ').trim()}|${Number(x.sale_price).toFixed(2)}`;
      if(seen.has(key)) return false;
      seen.add(key);
      return true;
  ...
  if(!items.length) throw new Error('전단에서 확인 가능한 상품/가격/위치를 추출하지 못했습니다.');

  const now=new Date().toISOString();
  const flyerPayload:any={
    region,
    business_id:businessId,
    title:standardSmartFlyerTitle(businessName,businessId),
    image_url:imageUrl,
    start_date:body.start_date||null,
    end_date:body.end_date||null,
    status:'draft',
  ...
  // V65: 설치 시점에 따라 optional column이 다를 수 있어 단계적으로 최소 스키마까지 재시도합니다.
  const flyerVariants:any[]=[
    {...flyerPayload},
    Object.fromEntries(Object.entries(flyerPayload).filter(([k])=>!['analysis_status'].includes(k))),
    Object.fromEntries(Object.entries(flyerPayload).filter(([k])=>!['analysis_status','updated_at','created_at'].includes(k))),
  ];
  let insert:any={data:null,error:null};
  const flyerErrors:string[]=[];
  for(const candidate of flyerVariants){
    insert=await admin.from('weekly_flyers').insert(candidate).select().single();
    if(!insert.error) break;
    flyerErrors.push(edgeErrorText(insert.error));
  }
  if(insert.error){
    throw new Error(`weekly_flyers 저장 실패: ${flyerErrors.join(' | ')}`);
  }
  const flyer=insert.data;
  ...
  const itemVariants:any[]=[
    itemRows.map(({sort_order,...rest}:any)=>rest),
    itemRows.map(({sort_order,ai_score,discount_percent,...rest}:any)=>rest),
    itemRows.map(({sort_order,ai_score,discount_percent,created_at,updated_at,...rest}:any)=>rest),
  ];
  let itemInsert:any={data:null,error:null};
  const itemErrors:string[]=[];
  for(const candidate of itemVariants){
    itemInsert=await admin.from('weekly_flyer_items').insert(candidate).select();
    if(!itemInsert.error) break;
    itemErrors.push(edgeErrorText(itemInsert.error));
  }
  if(itemInsert.error){
    await admin.from('weekly_flyers').delete().eq('id',flyer.id);
    throw new Error(`weekly_flyer_items 저장 실패: ${itemErrors.join(' | ')}`);
  }
  return {ok:true,version:VERSION,flyer,item_count:itemInsert.data?.length||items.length,items:itemInsert.data||items,analysis_passes:analysisPasses,rescue_used:rescueUsed};
}



async function saveWeeklyFlyerItemCrops(body:any){
  const flyerId=Number(body.flyer_id||body.id||0);
  if(!flyerId) throw new Error('전단 ID가 없습니다.');
  const crops=Array.isArray(body.crops)?body.crops:[];
  let updated=0;
  const failed:any[]=[];
  for(const crop of crops){
    const itemId=Number(crop?.item_id||0);
    if(!itemId) continue;
    const patch:any={
      item_image_url:String(crop.item_image_url||'').trim()||null,
      crop_status:String(crop.crop_status||'pending').trim(),
      crop_error:String(crop.crop_error||'').trim()||null,
      updated_at:new Date().toISOString()
    };
    let r=await admin.from('weekly_flyer_items').update(patch).eq('id',itemId).eq('flyer_id',flyerId);
    if(r.error && /updated_at/i.test(r.error.message||'')){
      delete patch.updated_at;
      r=await admin.from('weekly_flyer_items').update(patch).eq('id',itemId).eq('flyer_id',flyerId);
    }
    if(r.error) failed.push({item_id:itemId,error:edgeErrorText(r.error)});
    else updated++;
  }
  return {ok:true,version:VERSION,updated,failed};
}

async function publicWeeklyFlyers(region='dallas'){
  const today=dateKeyInDallas();
  const {data,error}=await admin.from('weekly_flyers')
    .select('*,weekly_flyer_items(*)')
    .eq('region',region)
    .eq('status','active')
  ...
  const flyers=(data||[]).filter((f:any)=>{
    const s=String(f.start_date||'').slice(0,10);
    const e=String(f.end_date||'').slice(0,10);
    return (!s||s<=today)&&(!e||e>=today);
  });
  return {ok:true,version:VERSION,flyers};
}

async function reanalyzeWeeklyFlyerPositions(body:any){
  const id=Number(body.id||0);
  if(!id) throw new Error('전단 ID가 없습니다.');
  const {data:flyer,error:fe}=await admin.from('weekly_flyers')
    .select('*,weekly_flyer_items(*)').eq('id',id).maybeSingle();
  if(fe) throw fe;
  if(!flyer) throw new Error('전단을 찾지 못했습니다.');
  if(!/^https?:\/\//i.test(String(flyer.image_url||''))) throw new Error('원본 전단 이미지 URL이 없습니다.');

  const items=Array.isArray(flyer.weekly_flyer_items)?flyer.weekly_flyer_items:[];
  if(!items.length) throw new Error('재분석할 상품이 없습니다.');
  const compact=items.slice(0,30).map((x:any)=>({
    id:x.id,
  ...
  const result:any=await openai({
    ...
    input:[{
      role:'user',
      content:[
        {type:'input_text',text:`Locate these already-extracted products on the attached supermarket flyer.
Products JSON: ${JSON.stringify(compact)}
Return ONLY JSON:
{"matches":[{"id":123,"source_box":{"x":0.0,"y":0.0,"width":0.0,"height":0.0},"confidence":90}]}
Rules:
- Match product name, printed sale price AND unit/pack quantity/membership conditions in the same block. Never match price alone.
- Return only confidence >= 90 matches; omit ambiguous or conflicting units.
- source_box coordinates are normalized 0..1 relative to the full original image.
- x=left, y=top, width and height are the product tile/area.
- Include product photo and printed price where practical, while avoiding neighboring products.
- Omit uncertain matches. No commentary.`},
        {type:'input_image',image_url:String(flyer.image_url),detail:'high'}
      ]
    }]
  },90000);
  ...
  for(const m of matches){
    const itemId=Number(m?.id||0);
    const box=normalizeFlyerSourceBox(m?.source_box);
    if(!itemId||!box||!Number.isFinite(Number(m.confidence))||Number(m.confidence)<90
      ||!compact.some((item:any)=>Number(item.id)===itemId)) continue;
    let r=await admin.from('weekly_flyer_items')
      .update({source_box:box,item_image_url:null,crop_status:'pending',crop_error:null,updated_at:new Date().toISOString()})
      .eq('id',itemId).eq('flyer_id',id);
    if(r.error && /updated_at/i.test(r.error.message||'')){
      r=await admin.from('weekly_flyer_items').update({source_box:box,item_image_url:null,crop_status:'pending',crop_error:null}).eq('id',itemId).eq('flyer_id',id);
    }
    if(r.error) failures.push({item_id:itemId,error:edgeErrorText(r.error)});
    else positionsUpdated++;
  }
  return {
...
function safeFlyerMainFileName(value='image.jpg'){
  const raw=String(value||'image.jpg').split(/[\\/]/).pop()||'image.jpg';
  const cleaned=raw.replace(/[^a-zA-Z0-9._-]+/g,'_').replace(/^_+|_+$/g,'');
  return cleaned.slice(-120)||'image.jpg';
}
function decodeBase64Data(value:any){
  const raw=String(value||'').trim();
  if(!raw) throw new Error('이미지 데이터가 없습니다.');
  ...
  try{
    ...
    throw new Error('이미지 Base64 데이터를 해석하지 못했습니다.');
  }
}
async function patchFlyerMainImage(flyerId:number,slot:number,url:string|null){
  const column=slot===2?'market_main_image_url_2':'market_main_image_url';
  const patch:any={[column]:url,updated_at:new Date().toISOString()};
  let r=await admin.from('weekly_flyers').update(patch).eq('id',flyerId).select().maybeSingle();
  if(r.error && /updated_at/i.test(r.error.message||'')){
    delete patch.updated_at;
    r=await admin.from('weekly_flyers').update(patch).eq('id',flyerId).select().maybeSingle();
  }
  if(r.error) throw new Error(`weekly_flyers.${column} 저장 실패: ${edgeErrorText(r.error)}`);
  if(!r.data) throw new Error('전단을 찾지 못했습니다.');
  return r.data;
}
async function saveWeeklyFlyerMainImage(body:any){
  const flyerId=Number(body.flyer_id||body.id||0);
  const slot=Number(body.slot||1)===2?2:1;
  if(!flyerId) throw new Error('전단 ID가 없습니다.');

  let url:any=null;
  if(body.market_main_image_url!==undefined){
    url=body.market_main_image_url===null?null:String(body.market_main_image_url||'').trim()||null;
  }else if(slot===2 && body.market_main_image_url_2!==undefined){
    url=body.market_main_image_url_2===null?null:String(body.market_main_image_url_2||'').trim()||null;
  }

  const flyer=await patchFlyerMainImage(flyerId,slot,url);
  return {
    ok:true,version:VERSION,flyer,
    market_main_image_url:flyer?.market_main_image_url||null,
    market_main_image_url_2:flyer?.market_main_image_url_2||null
  };
}
async function uploadWeeklyFlyerMainImage(body:any){
  const flyerId=Number(body.flyer_id||body.id||0);
  const slot=Number(body.slot||1)===2?2:1;
  if(!flyerId) throw new Error('전단 ID가 없습니다.');

  const {data:flyer,error:fe}=await admin.from('weekly_flyers')
    .select('id,region,business_id,market_main_image_url,market_main_image_url_2')
    .eq('id',flyerId).maybeSingle();
  if(fe) throw fe;
  if(!flyer) throw new Error('전단을 찾지 못했습니다.');

  const bytes=decodeBase64Data(body.image_base64);
  if(bytes.byteLength<500) throw new Error('이미지 파일이 너무 작습니다.');
  if(bytes.byteLength>8*1024*1024) throw new Error('메인 이미지는 8MB 이하로 올려주세요.');

  const contentType=String(body.content_type||'image/jpeg').toLowerCase();
  const allowed=['image/jpeg','image/png','image/webp'];
  if(!allowed.includes(contentType)) throw new Error('JPG, PNG, WebP 이미지만 업로드할 수 있습니다.');

  const ext=contentType==='image/png'?'png':contentType==='image/webp'?'webp':'jpg';
  const requested=safeFlyerMainFileName(body.file_name||`market-${slot}.${ext}`);
  const stem=requested.replace(/\.[^.]+$/,'').slice(0,80)||`market-${slot}`;
  const path=`weekly-flyer-main/${flyer.business_id||'business'}/${flyerId}/${Date.now()}-${slot}-${stem}.${ext}`;

  // Existing site uses public-images by default. Service Role uploads server-side.
  const bucket=String(env('STORAGE_BUCKET')||env('PUBLIC_STORAGE_BUCKET')||'public-images').trim()||'public-images';
  const upload=await admin.storage.from(bucket).upload(path,bytes,{
    contentType,
    cacheControl:'31536000',
    upsert:false
  });
  if(upload.error) throw new Error(`Storage 업로드 실패: ${edgeErrorText(upload.error)}`);

  const publicData=admin.storage.from(bucket).getPublicUrl(path);
  const publicUrl=String(publicData?.data?.publicUrl||'').trim();
  if(!/^https?:\/\//i.test(publicUrl)){
    await admin.storage.from(bucket).remove([path]).catch(()=>{});
    throw new Error('업로드 후 공개 이미지 URL을 만들지 못했습니다.');
  }

  try{
    const updated=await patchFlyerMainImage(flyerId,slot,publicUrl);
    return {
      ok:true,version:VERSION,flyer:updated,bucket,path,
      market_main_image_url:updated?.market_main_image_url||null,
      market_main_image_url_2:updated?.market_main_image_url_2||null
    };
  }catch(e){
    await admin.storage.from(bucket).remove([path]).catch(()=>{});
    throw e;
...
async function status(region = 'dallas') {
  ...
  return {
    ...
    supported_actions: ['status', 'run_status', 'test_post', 'publish_one', 'cleanup', 'collect', 'analyze', 'draft', 'get_settings', 'save_settings', 'version', 'ping', 'home_feed', 'home_settings', 'list_scheduled_topics', 'save_scheduled_topic', 'delete_scheduled_topic', 'collect_scheduled_topics', 'auto_run', 'set_editor_pick', 'set_home_link', 'set_archive_keep', 'delete_newsroom_item', 'trace_sources', 'collect_markets', 'daily_dallas_life', 'automation_status', 'analyze_weekly_flyer', 'list_weekly_flyers', 'set_weekly_flyer_status', 'delete_weekly_flyer', 'update_weekly_flyer_items', 'activate_weekly_flyer', 'reanalyze_weekly_flyer_positions', 'save_weekly_flyer_item_crops', 'public_weekly_flyers', 'upload_weekly_flyer_main_image', 'save_weekly_flyer_main_image'],
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    if (action === 'public_weekly_flyers') return json(await publicWeeklyFlyers(region));

    const auth = await authorize(req);
    if (action === 'analyze_weekly_flyer') return json(await analyzeWeeklyFlyer({...body,region}));
    if (action === 'upload_weekly_flyer_main_image') return json(await uploadWeeklyFlyerMainImage(body));
    if (action === 'save_weekly_flyer_main_image') return json(await saveWeeklyFlyerMainImage(body));
    if (action === 'reanalyze_weekly_flyer_positions') return json(await reanalyzeWeeklyFlyerPositions({...body,region}));
    if (action === 'save_weekly_flyer_item_crops') return json(await saveWeeklyFlyerItemCrops(body));

    if (action === 'list_weekly_flyers') return json(await listWeeklyFlyers({...body,region}));
    if (action === 'set_weekly_flyer_status') return json(await setWeeklyFlyerStatus(body));
    if (action === 'delete_weekly_flyer') return json(await deleteWeeklyFlyer(body));
    if (action === 'update_weekly_flyer_items') return json(await updateWeeklyFlyerItems(body));
    if (action === 'activate_weekly_flyer') return json(await activateWeeklyFlyer(body));
    if (action === 'status' || action === 'health') return json(await status(region));
    if (action === 'run_status') return json(await runStatus(region));
    if (action === 'automation_status') return json(await automationStatus(region));
    ...
    if (action === 'daily_dallas_life') return json(await dailyDallasLife(region));
    ...
    const message=edgeErrorText(e);
    console.error('[newsroom V65]',message,e);
    return json({
      ok:false,
      version:VERSION,
      error:message,
      error_type:(e as any)?.constructor?.name||typeof e,
