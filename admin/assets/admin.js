...
(function(){
  ...
  async function save(){
    ...
    status.textContent='이미지를 업로드하고 전단에 연결하고 있습니다.';
    ...
    try{
      ...
      if(!/^https?:\/\//i.test(savedUrl)){
        throw new Error('메인 이미지 URL이 DB에 저장되지 않았습니다.');
      }

      status.textContent='메인 이미지와 전단 연결을 저장했습니다.';
      try{
      ...
      if(window.P010SmartFlyer?.load){
        setTimeout(()=>window.P010SmartFlyer.load(),200);
      }
    }catch(error){
  ...
  document.addEventListener('DOMContentLoaded',()=>{
    setTimeout(()=>{
      const client=
        ...
        null;
      console.info('[DalTownMap] P060 full admin restored with market image REST upload',Boolean(client?.storage));
    },800);
  });
})();
...
console.info('[DalTownMap] P064 atomic Edge market image upload loaded');
