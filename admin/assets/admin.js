...
async function v48SetEditorPick(id,enabled=true){try{await newsroomEdgeCall('set_editor_pick',{id,enabled,region:getAppRegion()});await loadNewsroom();safeText('newsroomStatus',enabled?'공개 후보로 지정했습니다. 메인 링크를 승인해야 실제 메인에 노출됩니다.':'관리자 지정을 해제했습니다.');}catch(e){alert(e.message);}}
