...
(() => {
  ...
  async function upload(file){
    ...
    // 1순위: 실제 Supabase client가 노출되어 있으면 SDK 사용
    ...
    // 2순위: SDK client가 없어도 Supabase Storage REST API로 직접 업로드
  ...
  document.addEventListener('DOMContentLoaded',()=>{
    setTimeout(()=>{
      const client=
        ...
        null;
      console.info('[DalTownMap] P059 market image uploader',{sdk: Boolean(client?.storage), rest: Boolean(getP057Config().base&&getP057Config().anon)});
    },800);
  });
})();
...
// === P059: Storage REST 업로드 fallback ===
(() => {
  document.addEventListener('DOMContentLoaded',()=>{
    setTimeout(()=>{
      const cfg=
        ...
        {};
      console.info('[DalTownMap] P059 storage fallback ready',{
        hasUrl:Boolean(cfg.SUPABASE_URL),
        hasAnon:Boolean(cfg.SUPABASE_ANON_KEY)
      });
    },900);
  });
})();
