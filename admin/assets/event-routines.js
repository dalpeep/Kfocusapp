...
async function saveForm(){try{const r=payload(),i=routines.findIndex(x=>String(x.id)===String(r.id));const isEdit=i>=0;if(isEdit)routines[i]={...routines[i],...r};else routines.push({...r,created_at:new Date().toISOString()});await save();edit(r.id);logActivity(`${r.name} ${isEdit?'수정':'추가'} · ${Object.keys(r.actions).map(k=>ACTION_LABELS[k]).join(', ')}`,isEdit?'updated':'created');alert('이벤트 루틴을 저장했습니다. 달타운 추천은 업소 데이터로, 달타운 알림은 게시판 공지로 운영됩니다.');}catch(e){alert(e.message);}}
