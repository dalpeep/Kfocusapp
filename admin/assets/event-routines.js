...
const OPTION_LABELS={
 recommendation:{recommended:'추천 업체',new:'신규 업체',popular:'인기 업체',coupon:'쿠폰 있는 업체',banner:'배너 있는 업체',address:'주소 기반 업체',admin:'관리자 지정 업체',random:'랜덤 업체'},
 alert:{board_notice:'게시판 공지',business_new:'신규 업체',business_popular:'인기 업체',business_rating:'평점 높은 업체',business_ad:'광고 업체',business_ai:'AI 추천',business_random:'랜덤'},
...
function actionSummary(a,k){const selected=(a.options||[]).map(v=>OPTION_LABELS[k]?.[v]||v);const custom=a.custom_items||[];const addressTerms=Array.isArray(a.address_terms)?a.address_terms:[];const parts=[];if(selected.length)parts.push(selected.join(' · '));if(addressTerms.length)parts.push(`주소 ${addressTerms.join(', ')}`);if(custom.length)parts.push(`직접 문구 ${custom.length}개`);return parts.join(' + ')||a.detail||'기본 설정';}
...
function payload(){
 const name=$('erName').value.trim();if(!name)throw new Error('이벤트 이름을 입력하세요.');
 const actions={};
 document.querySelectorAll('#erActionGrid>label').forEach(l=>{
  const cb=l.querySelector(':scope > input[type=checkbox]');if(!cb.checked)return;
  const optionBox=l.querySelector('[data-er-options]');
  let options=optionBox?[...optionBox.querySelectorAll('input[type=checkbox]:checked')].map(x=>x.value):[];
  if(cb.value==='alert'&&!options.length)options=['board_notice'];
  const custom=l.querySelector('[data-er-custom]');
  const customItems=(custom?.value||'').split('\n').map(x=>x.trim()).filter(Boolean).map(text=>({text}));
  const address=l.querySelector('[data-er-address]');
  const addressTerms=(address?.value||'').split(/\r?\n|,/).map(x=>x.trim()).filter(Boolean);
  if(cb.value==='recommendation'&&options.includes('address')&&!addressTerms.length)throw new Error('주소 기반 업체를 선택한 경우 주소 검색어를 입력하세요.');
  if(!options.length&&!customItems.length)throw new Error(`${ACTION_LABELS[cb.value]}에서 유형을 선택하세요.`);
  const interval=l.querySelector('[data-er-interval]');
  const linkType=l.querySelector('[data-er-link-type]')?.value||'none';
  const linkValue=l.querySelector('[data-er-link-value]')?.value.trim()||'';
  const action={options,custom_items:customItems,address_terms:addressTerms,interval_seconds:Number(interval?.value||5),mode:'rotate',link_type:linkType,link_value:linkValue};
  action.detail=actionSummary(action,cb.value);actions[cb.value]=action;
 });
 if(!Object.keys(actions).length)throw new Error('실행할 영역을 하나 이상 선택하세요.');
 const t=$('erScheduleType').value,start=$('erStartAt').value,end=$('erEndAt').value;
 if(['range','today'].includes(t)&&(!start||!end))throw new Error('시작과 종료 날짜를 입력하세요.');
 if(start&&end&&new Date(end)<new Date(start))throw new Error('종료 날짜는 시작 날짜보다 빠를 수 없습니다.');
 return{id:$('erId').value||`event-${Date.now()}-${Math.random().toString(36).slice(2,7)}`,name,description:$('erDescription').value.trim(),region:$('erRegion').value,schedule_type:t,enabled:$('erEnabled').value==='true',start_at:start||null,end_at:t==='open'?null:(end||null),repeat_type:$('erRepeatType').value,repeat_value:$('erRepeatValue').value.trim(),actions,updated_at:new Date().toISOString()};
}
async function saveForm(){try{const r=payload(),i=routines.findIndex(x=>String(x.id)===String(r.id));const isEdit=i>=0;if(isEdit)routines[i]={...routines[i],...r};else routines.push({...r,created_at:new Date().toISOString()});await save();edit(r.id);logActivity(`${r.name} ${isEdit?'수정':'추가'} · ${Object.keys(r.actions).map(k=>ACTION_LABELS[k]).join(', ')}`,isEdit?'updated':'created');alert('이벤트 루틴을 저장했습니다. 달타운 알림은 게시판 공지 → 직접 입력·업소 조건 순서로 적용됩니다.');}catch(e){alert(e.message);}}
