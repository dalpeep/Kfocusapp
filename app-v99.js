...
console.log('[DalTownMap] P001 life subcategory patch loaded');
...
function inferLifeCategory(post={}){
  ...
  // P001: 달라스 라이프 소분류 정교화
  if(/도로|교통|차량|운전|공항|버스|열차|통제|폐쇄|우회|정체|사고|공사|고속도로|톨웨이|txdot|dart|traffic|road|highway|closure|transit|airport/.test(text)) return '교통';
  ...
  if(/쇼핑|마트|마켓|세일|할인|특가|장보기|신규 매장|매장 오픈|오픈 소식|개점|신규 식당|식당 오픈|레스토랑|카페|베이커리|h마트|시온|코마트|store|shopping|market|grocery|sale|discount|grand opening|restaurant|cafe|bakery/.test(text)) return '쇼핑';
  if(/경찰|소방|범죄|체포|실종|경보|주의보|대피|총격|화재|폭염|홍수|토네이도|안전|위험|비상|police|fire|crime|arrest|missing|alert|warning|evacuation|shooting|heat advisory|flood|tornado|safety/.test(text)) return '안전';
