...
// === P012: 스마트 전단 앱 메인 최종 연결 ===
(() => {
  let p012Flyers=[];
  let p012LoadedAt=0;
  let p012Loading=null;
  ...
  function p012Valid(f){
    const day=p012Today();
    return String(f?.status||'')==='active' &&
      f?.show_on_home!==false &&
      (!f?.start_date||String(f.start_date)<=day) &&
      (!f?.end_date||String(f.end_date)>=day);
  }
  async function p012Load(force=false){
    if(!force&&Date.now()-p012LoadedAt<45*1000)return p012Flyers;
    if(p012Loading)return p012Loading;
    p012Loading=(async()=>{
      try{
        const {data,error}=await supabase.from('weekly_flyers')
          .select('*,weekly_flyer_items(*)')
          ...
          .limit(20);
        if(error)throw error;
        p012Flyers=(data||[]).filter(p012Valid);
        p012LoadedAt=Date.now();
        return p012Flyers;
      }catch(e){
        console.warn('[P012 flyer load]',e?.message||e);
        return p012Flyers;
      }finally{p012Loading=null;}
    })();
    return p012Loading;
  }
  function p012Items(f){
    return (Array.isArray(f?.weekly_flyer_items)?f.weekly_flyer_items:[])
      .slice().sort((a,b)=>
        Number(b.is_featured||0)-Number(a.is_featured||0)||
        Number(b.ai_score||0)-Number(a.ai_score||0)
  ...
  function p012Money(v){
    const n=Number(v);
    return Number.isFinite(n)?`$${n.toFixed(2)}`:'';
  }
  function p012HomeItem(f){
    const items=p012Items(f).slice(0,4);
    return {
      id:`smart-flyer-${f.id}`,
      source_id:`smart-flyer-${f.id}`,
      category:'market',
      category_label:'이번 주 특가',
      icon:'🛒',
      title:f.title||'이번 주 마켓 세일',
      summary:items.map(x=>`${x.product_name}${x.sale_price!=null?` ${p012Money(x.sale_price)}`:''}`).join(' · ')||f.ai_summary||'이번 주 세일을 확인하세요.',
      subtitle:items.map(x=>`${x.product_name}${x.sale_price!=null?` ${p012Money(x.sale_price)}`:''}`).join(' · '),
      target_type:'business',
      target_id:f.business_id,
      business_id:f.business_id,
      link_label:'전체 세일 보기 →',
  ...
  const p012BasePrepare=typeof v51PrepareTodayItems==='function'?v51PrepareTodayItems:null;
  if(p012BasePrepare){
    v51PrepareTodayItems=function(items=[]){
      const flyerItems=p012Flyers.map(p012HomeItem);
      const combined=[...flyerItems,...(items||[]).filter(x=>!String(x?.id||'').startsWith('smart-flyer-'))];
      return p012BasePrepare(combined);
    };
  }

  async function p012Refresh(force=false){
    await p012Load(force);
    try{localStorage.removeItem('daltownmap_v38_home');}catch{}
    if(typeof v51RefreshToday==='function')await v51RefreshToday();
    if(window.P010SmartFlyerPublic?.refresh)await window.P010SmartFlyerPublic.refresh(force);
  }

  document.addEventListener('DOMContentLoaded',()=>{
    setTimeout(()=>p012Refresh(true),1000);
    setTimeout(()=>p012Refresh(true),3200);
    setInterval(()=>{if(!document.hidden)p012Refresh(true);},45*1000);
  });
  document.addEventListener('visibilitychange',()=>{
    if(document.visibilityState==='visible')p012Refresh(true);
  });
  window.addEventListener('focus',()=>p012Refresh(true));
  window.addEventListener('storage',e=>{
    if(e.key==='daltownmap_content_changed')p012Refresh(true);
  });
  try{
    const bc=new BroadcastChannel('daltownmap-content');
    bc.addEventListener('message',()=>p012Refresh(true));
  }catch{}

  window.P012SmartFlyerHome={refresh:p012Refresh};
...
// === P019: 스마트 전단 최종 메인 UI · 화살표·스와이프·AI 추천 ===
(() => {
  const STATE = {
    timer: null,
    index: 0,
  ...
  function ensureStyle(){
    if(document.getElementById('p019SmartFlyerFinalStyle'))return;
    const s=document.createElement('style');
    s.id='p019SmartFlyerFinalStyle';
    s.textContent=`
      #v37BriefCard.p019-smart-flyer-final{
        position:relative;
        height:286px !important;
        min-height:286px !important;
        max-height:286px !important;
        overflow:hidden;
        background:linear-gradient(145deg,#b75b05 0%,#d57a03 52%,#a94d00 100%);
      }
      #v37BriefCard.p019-smart-flyer-final::after{display:none!important}
      #v37BriefCard.p019-smart-flyer-final .p014-flyer-thumb,
      #v37BriefCard.p019-smart-flyer-final .p015-flyer-photo,
      #v37BriefCard.p019-smart-flyer-final .p015-flyer-shade,
      #v37BriefCard.p019-smart-flyer-final .p015-flyer-bottom,
      #v37BriefCard.p019-smart-flyer-final .p016-slider{
        display:none!important;
      }
      #v37BriefCard .p019-wrap{
        position:absolute;
        z-index:8;
        left:12px;
        right:12px;
      ...
      #v37BriefCard .p019-product{
        position:relative;
        overflow:hidden;
        border-radius:14px;
        background:#fff;
        color:#111827;
        box-shadow:0 8px 18px rgba(66,28,1,.22);
        display:flex;
        flex-direction:column;
        min-width:0;
      }
      #v37BriefCard .p019-product-image{
        position:relative;
        height:123px;
        overflow:hidden;
        background:#f8fafc;
      }
      #v37BriefCard .p019-product-image img{
        width:100%;
        height:100%;
        object-fit:cover;
        object-position:center;
      ...
      #v37BriefCard .p019-info{
        padding:7px 9px 8px;
        min-height:48px;
      }
      #v37BriefCard .p019-name{
        display:-webkit-box;
        -webkit-line-clamp:2;
        -webkit-box-orient:vertical;
        overflow:hidden;
      ...
      #v37BriefCard .p019-price{
        margin-top:3px;
        color:#dc2626;
        font-size:15px;
        font-weight:950;
      }
      #v37BriefCard .p019-nav{
        position:absolute;
        z-index:10;
        top:50%;
      ...
      #v37BriefCard .p019-prev{left:-4px}
      #v37BriefCard .p019-next{right:-4px}
      #v37BriefCard .p019-foot{
        display:flex;
        justify-content:space-between;
        align-items:center;
        gap:9px;
      ...
      #v37BriefCard .p019-dots{
        display:flex;
        align-items:center;
        gap:5px;
      }
      #v37BriefCard .p019-dot{
        width:6px;
        height:6px;
        border-radius:999px;
      ...
      #v37BriefCard .p019-dot.active{
        width:18px;
        background:#fff;
      }
      #v37BriefCard .p019-more{
        white-space:nowrap;
      }
      @media(max-width:390px){
        #v37BriefCard.p019-smart-flyer-final{
        ...
        #v37BriefCard .p019-wrap{left:10px;right:10px}
        #v37BriefCard .p019-product-image{height:117px}
        #v37BriefCard .p019-name{font-size:10.5px}
        #v37BriefCard .p019-price{font-size:14px}
        #v37BriefCard .p019-nav{width:31px;height:31px;font-size:20px}
      }
    `;
    document.head.appendChild(s);
  }
  ...
  async function fetchFlyer(flyerId){
    try{
      ...
      return (json.flyers||[]).find(f=>Number(f.id)===Number(flyerId))||null;
    }catch(error){
      console.warn('[P019 flyer fetch]',error);
      return null;
    }
  }
  ...
  function sortedItems(flyer){
    return (Array.isArray(flyer?.weekly_flyer_items)?flyer.weekly_flyer_items:[])
      .filter(x=>x.item_image_url&&String(x.crop_status||'')==='complete')
      .slice()
      .sort((a,b)=>
        Number(b.is_featured||0)-Number(a.is_featured||0) ||
        Number(b.ai_score||0)-Number(a.ai_score||0) ||
  ...
  function draw(card,item){
    card.querySelectorAll('.p019-wrap').forEach(n=>n.remove());
    const pages=Math.max(1,Math.ceil(STATE.items.length/2));
    STATE.index=((STATE.index%pages)+pages)%pages;
    const products=STATE.items.slice(STATE.index*2,STATE.index*2+2);

    const wrap=document.createElement('div');
    wrap.className='p019-wrap';
    wrap.innerHTML=`
      <div class="p019-head">
        <div class="p019-head-left">
          <span>🛒 이번 주 특가</span>
          <span class="p019-live">LIVE</span>
        </div>
        <div class="p019-market">${safeText(item?.title||'마켓 세일')} →</div>
      </div>

      <div class="p019-stage">
        ${products.map((p,i)=>`
          <article class="p019-product">
            <div class="p019-product-image">
              ${i===0&&Number(p.is_featured)?'<span class="p019-ai-badge">AI 추천</span>':''}
              <img src="${String(p.item_image_url||'').replace(/"/g,'')}" alt="${safeText(p.product_name||'상품')}">
            </div>
            <div class="p019-info">
              <div class="p019-name">${safeText(p.product_name||'세일 상품')}</div>
              <div class="p019-price">${money(p.sale_price)}</div>
            </div>
          </article>`).join('')}

        ${pages>1?`
          <button type="button" class="p019-nav p019-prev" aria-label="이전 상품">‹</button>
          <button type="button" class="p019-nav p019-next" aria-label="다음 상품">›</button>`:''}
      </div>

      <div class="p019-foot">
        <div class="p019-dots">
          ${Array.from({length:pages},(_,i)=>`<span class="p019-dot ${i===STATE.index?'active':''}"></span>`).join('')}
        </div>
        <span class="p019-more">나머지 상품은 상세에서 확인 →</span>
      </div>`;

    card.appendChild(wrap);

    const move=delta=>{
      STATE.index=(STATE.index+delta+pages)%pages;
      draw(card,item);
      restartAuto(card,item);
    };

    wrap.querySelector('.p019-prev')?.addEventListener('click',e=>{
      e.stopPropagation();
      move(-1);
    });
    wrap.querySelector('.p019-next')?.addEventListener('click',e=>{
      e.stopPropagation();
      move(1);
    });
  ...
  function restartAuto(card,item){
    stopTimer();
    const pages=Math.ceil(STATE.items.length/2);
    if(pages<=1)return;
    STATE.timer=setInterval(()=>{
      const current=Array.isArray(v51TodayItems)?v51TodayItems[v51TodayIndex]:null;
      if(!current||Number(current.weekly_flyer_id)!==Number(STATE.flyerId)){
        stopTimer();
        return;
      }
      STATE.index=(STATE.index+1)%pages;
      draw(card,current);
    },3800);
  }
  ...
  v51PaintToday=async function(){
    ...
    card.classList.remove('p019-smart-flyer-final');
    card.querySelectorAll('.p019-wrap').forEach(n=>n.remove());

    if(!isFlyer(item))return;

    const flyer=await fetchFlyer(item.weekly_flyer_id);
    const products=sortedItems(flyer);
    if(!products.length)return; // 기존 원본 전단 카드가 fallback으로 남습니다.

    STATE.flyerId=item.weekly_flyer_id;
    STATE.items=products;
    STATE.index=0;

    card.classList.add('p019-smart-flyer-final');
    draw(card,item);
    restartAuto(card,item);
  };
...
(() => {
  ...
  function ensureStyle(){
    if(document.getElementById('p024FinalLayoutStyle'))return;
    const s=document.createElement('style');
    s.id='p024FinalLayoutStyle';
    s.textContent=`
      #v37BriefCard.p024-flyer-card{
        ...
        background:linear-gradient(145deg,#b85b05 0%,#d47a04 52%,#a94e00 100%)!important;
      }

      #v37BriefCard.p024-flyer-card > *:not(.p024-shell){
        visibility:hidden!important;
        pointer-events:none!important;
      }

      #v37BriefCard.p024-flyer-card::before,
      #v37BriefCard.p024-flyer-card::after{
        display:none!important;
      }

      #v37BriefCard .p024-shell{
        position:absolute!important;
        inset:0!important;
        z-index:999!important;
        display:grid!important;
        grid-template-rows:42px minmax(0,1fr) 28px!important;
      ...
      #v37BriefCard .p024-product{
        min-width:0;
        overflow:hidden;
        border-radius:14px;
        background:#fff;
        color:#111827;
        box-shadow:0 8px 18px rgba(67,28,0,.22);
        display:grid;
        grid-template-rows:minmax(0,1fr) 52px;
      }

      #v37BriefCard .p024-image{
        position:relative;
        min-height:0;
        overflow:hidden;
        background:#f8fafc;
      ...
      #v37BriefCard .p024-info{
        padding:7px 9px 8px;
        box-sizing:border-box;
      }

      #v37BriefCard .p024-name{
        overflow:hidden;
        white-space:nowrap;
        text-overflow:ellipsis;
        font-size:11px;
      ...
      #v37BriefCard .p024-prev{left:-5px}
      #v37BriefCard .p024-next{right:-5px}

      #v37BriefCard .p024-foot{
        display:flex;
      ...
      #v37BriefCard .p024-dots{
        display:flex;
        align-items:center;
        gap:5px;
      }

      #v37BriefCard .p024-dot{
        width:6px;
        height:6px;
        border-radius:99px;
        background:rgba(255,255,255,.38);
      ...
      #v37BriefCard .p024-dot.active{
        width:18px;
        background:#fff;
      }

      #v37BriefCard .p024-more{
        white-space:nowrap;
      }
      ...
      @media(max-width:390px){
        ...
        #v37BriefCard .p024-shell{
          grid-template-rows:40px minmax(0,1fr) 26px!important;
  ...
  async function fetchFlyer(flyerId){
    ...
    try{
      ...
      console.warn('[P024 flyer fetch]',error);
      return null;
    }
  }

  function sorted(flyer){
    return (Array.isArray(flyer?.weekly_flyer_items)?flyer.weekly_flyer_items:[])
      .filter(x=>x.item_image_url&&String(x.crop_status||'')==='complete')
      .slice()
      .sort((a,b)=>
  ...
  function draw(card){
    if(!card||!STATE.item||!STATE.items.length)return;

    card.querySelectorAll('.p024-shell').forEach(n=>n.remove());
    card.classList.add('p024-flyer-card');

    const pages=Math.max(1,Math.ceil(STATE.items.length/2));
    STATE.page=((STATE.page%pages)+pages)%pages;
    const pageItems=STATE.items.slice(STATE.page*2,STATE.page*2+2);

    const shell=document.createElement('div');
    shell.className='p024-shell';
    shell.innerHTML=`
      <div class="p024-head">
        <div class="p024-left">
          <span>🛒 이번 주 특가</span>
          <span class="p024-live">LIVE</span>
        </div>
        <div class="p024-market">${safe(STATE.item.title||'마켓 세일')} →</div>
      </div>

      <div class="p024-stage">
        ${pageItems.map((p,i)=>`
          <article class="p024-product">
            <div class="p024-image">
              ${i===0&&Number(p.is_featured)?'<span class="p024-ai">AI 추천</span>':''}
              <img src="${String(p.item_image_url||'').replace(/"/g,'')}" alt="${safe(p.product_name||'상품')}">
            </div>
            <div class="p024-info">
              <div class="p024-name">${safe(p.product_name||'세일 상품')}</div>
              <div class="p024-price">${money(p.sale_price)}</div>
            </div>
          </article>`).join('')}

        ${pages>1?`
          <button type="button" class="p024-nav p024-prev" aria-label="이전 상품">‹</button>
          <button type="button" class="p024-nav p024-next" aria-label="다음 상품">›</button>`:''}
      </div>

      <div class="p024-foot">
        <div class="p024-dots">
          ${Array.from({length:pages},(_,i)=>`<span class="p024-dot ${i===STATE.page?'active':''}"></span>`).join('')}
        </div>
        <span class="p024-more">나머지 상품은 상세에서 확인 →</span>
      </div>`;
    ...
    shell.querySelector('.p024-prev')?.addEventListener('click',e=>{
      e.stopPropagation();
      move(-1);
    });

    shell.querySelector('.p024-next')?.addEventListener('click',e=>{
      e.stopPropagation();
      move(1);
    });
    ...
    shell.addEventListener('touchend',e=>{
      ...
      const dy=t.clientY-STATE.touchY;
      if(Math.abs(dx)<38||Math.abs(dx)<Math.abs(dy))return;
      move(dx<0?1:-1);
    },{passive:true});
  }
  ...
  function restart(card){
    ...
    STATE.timer=setInterval(()=>{
      const current=Array.isArray(window.v51TodayItems)
        ? window.v51TodayItems[window.v51TodayIndex]
        : null;
  ...
  async function render(){
    ...
    try{
      ...
      const card=document.getElementById('v37BriefCard');
      const item=Array.isArray(window.v51TodayItems)
        ? window.v51TodayItems[window.v51TodayIndex]
        : null;

      if(!card)return;

      card.classList.remove('p024-flyer-card');
      card.querySelectorAll('.p024-shell').forEach(n=>n.remove());

      if(!isFlyer(item))return;

      const flyer=await fetchFlyer(item.weekly_flyer_id);
      const items=sorted(flyer);

      if(!items.length)return;

      STATE.flyerId=Number(item.weekly_flyer_id);
      STATE.page=0;
      STATE.items=items;
      STATE.item=item;
  ...
  const previousPaint=typeof window.v51PaintToday==='function'
    ? window.v51PaintToday
    : null;

  if(previousPaint){
    window.v51PaintToday=function(){
      const result=previousPaint();
      setTimeout(render,0);
      return result;
    };
  }
  ...
  window.P024SmartFlyerLayout={render};
  console.info('[DalTownMap] P024 Smart Flyer final layout collision fix loaded');
})();


// === P024: YouTube-style Smart Flyer single-product card ===
// One product per slide, de-duplicated by product identity, and matched to the
// current home recommendation card height.
(() => {
  const STATE = {
    timer: null,
    flyerId: null,
  ...
  function ensureStyle(){
    if(document.getElementById('p024SmartFlyerStyle')) return;
    const style=document.createElement('style');
    style.id='p024SmartFlyerStyle';
    style.textContent=`
      #v37BriefCard.p024-flyer-card{
        position:relative!important;
        height:var(--p024-recommend-height,132px)!important;
        min-height:var(--p024-recommend-height,132px)!important;
        max-height:var(--p024-recommend-height,132px)!important;
        padding:0!important;
        overflow:hidden!important;
        border-radius:22px!important;
      ...
      #v37BriefCard.p024-flyer-card::after{display:none!important}
      #v37BriefCard.p024-flyer-card > :not(.p024-flyer-wrap){display:none!important}
      #v37BriefCard .p024-flyer-wrap{
        position:absolute;inset:0;z-index:50;display:grid;
        grid-template-columns:42% minmax(0,1fr);background:#fff;
        touch-action:pan-y;cursor:pointer;
      }
      #v37BriefCard .p024-media{position:relative;overflow:hidden;background:#f3f4f6}
      #v37BriefCard .p024-media img{width:100%;height:100%;display:block;object-fit:cover;object-position:center}
      #v37BriefCard .p024-ad-badge{position:absolute;left:7px;top:7px;padding:3px 6px;border-radius:5px;background:rgba(0,0,0,.72);color:#fff;font-size:9px;font-weight:900}
      #v37BriefCard .p024-copy{min-width:0;padding:11px 38px 10px 12px;display:flex;flex-direction:column;justify-content:center}
      #v37BriefCard .p024-store{font-size:10px;font-weight:850;color:#6b7280;white-space:nowrap;overflow:hidden;text-overflow:ellipsis}
      #v37BriefCard .p024-name{margin-top:5px;font-size:15px;line-height:1.25;font-weight:900;color:#111827;display:-webkit-box;-webkit-box-orient:vertical;-webkit-line-clamp:2;overflow:hidden}
      #v37BriefCard .p024-price-row{display:flex;align-items:baseline;gap:7px;margin-top:6px;min-height:20px}
      #v37BriefCard .p024-sale{font-size:18px;line-height:1;font-weight:950;color:#dc2626}
      #v37BriefCard .p024-regular{font-size:10px;color:#9ca3af;text-decoration:line-through}
      #v37BriefCard .p024-unit{margin-top:4px;font-size:10px;color:#6b7280;white-space:nowrap;overflow:hidden;text-overflow:ellipsis}
      #v37BriefCard .p024-more{margin-top:5px;font-size:10px;font-weight:800;color:#2563eb}
      #v37BriefCard .p024-nav{position:absolute;right:7px;width:27px;height:27px;border:0;border-radius:50%;display:grid;place-items:center;background:rgba(255,255,255,.96);color:#1f2937;box-shadow:0 3px 10px rgba(15,23,42,.16);font-size:18px;line-height:1;z-index:3;cursor:pointer}
      #v37BriefCard .p024-prev{top:calc(50% - 30px)}
      #v37BriefCard .p024-next{top:calc(50% + 3px)}
      #v37BriefCard .p024-count{position:absolute;right:8px;bottom:6px;padding:2px 6px;border-radius:999px;background:#eef2f7;color:#64748b;font-size:9px;font-weight:900}
      @media(max-width:390px){
        #v37BriefCard .p024-flyer-wrap{grid-template-columns:40% minmax(0,1fr)}
        #v37BriefCard .p024-copy{padding-left:10px;padding-right:35px}
        #v37BriefCard .p024-name{font-size:14px}
        #v37BriefCard .p024-sale{font-size:17px}
      }
    `;
    document.head.appendChild(style);
  }
  ...
  function syncHeight(card){
    ...
    const measured=Math.round(recommend?.getBoundingClientRect?.().height||0);
    card.style.setProperty('--p024-recommend-height',`${Math.max(120,measured||132)}px`);
  }
  ...
  async function fetchFlyer(flyerId){
    try{
      ...
      const res=await fetch(`${base}/functions/v1/${encodeURIComponent(fn)}?action=public_weekly_flyers&region=${encodeURIComponent(region)}&_=${Date.now()}`,{
        headers:{apikey:key,Authorization:`Bearer ${key}`},cache:'no-store'
      });
      const json=await res.json().catch(()=>({}));
      ...
      console.warn('[P024 flyer fetch]',error?.message||error);
      return null;
    }
  }

  function normalizedItems(flyer){
    const rows=(Array.isArray(flyer?.weekly_flyer_items)?flyer.weekly_flyer_items:[])
      .filter(x=>x&&x.item_image_url&&String(x.crop_status||'complete')==='complete')
      .slice()
      .sort((a,b)=>
        Number(b.is_featured||0)-Number(a.is_featured||0)||
        Number(b.ai_score||0)-Number(a.ai_score||0)||
    ...
    return rows.filter(row=>{
      const name=safe(row.product_name).toLowerCase();
      const price=Number.isFinite(Number(row.sale_price))?Number(row.sale_price).toFixed(2):'';
      const image=String(row.item_image_url||'').split('?')[0].toLowerCase();
      const box=[row.crop_x,row.crop_y,row.crop_width,row.crop_height].map(v=>Number(v)||0).join(':');
      const key=name?`${name}|${price}`:`${image}|${box}`;
      if(seen.has(key)) return false;
      seen.add(key);
      return true;
    }).slice(0,12);
  ...
  function openFlyer(item){
    if(window.P010SmartFlyerPublic?.openModal&&item?.weekly_flyer_id){
      window.P010SmartFlyerPublic.openModal(item.weekly_flyer_id);
      return;
    }
    const businessId=item?.business_id||item?.target_id;
    if(businessId){
      selectedBizId=String(businessId);
  ...
  function draw(card,item){
    card.querySelectorAll('.p024-flyer-wrap').forEach(node=>node.remove());
    if(!STATE.items.length) return;
    STATE.index=((STATE.index%STATE.items.length)+STATE.items.length)%STATE.items.length;
    const product=STATE.items[STATE.index];
    const wrap=document.createElement('div');
    wrap.className='p024-flyer-wrap';
    wrap.innerHTML=`
      <div class="p024-media">
        <img src="${String(product.item_image_url||'').replace(/"/g,'')}" alt="${safe(product.product_name||'세일 상품')}">
        <span class="p024-ad-badge">광고</span>
      </div>
      <div class="p024-copy">
        <div class="p024-store">${safe(item?.title||'이번 주 세일')}</div>
        <div class="p024-name">${safe(product.product_name||'세일 상품')}</div>
        <div class="p024-price-row">
          ${product.sale_price!=null?`<strong class="p024-sale">${money(product.sale_price)}</strong>`:''}
          ${product.regular_price!=null?`<span class="p024-regular">${money(product.regular_price)}</span>`:''}
        </div>
        ${product.unit_text?`<div class="p024-unit">${safe(product.unit_text)}</div>`:''}
        <div class="p024-more">자세히 보기 →</div>
      </div>
      ${STATE.items.length>1?`<button type="button" class="p024-nav p024-prev" aria-label="이전 상품">‹</button><button type="button" class="p024-nav p024-next" aria-label="다음 상품">›</button>`:''}
      <span class="p024-count">${STATE.index+1}/${STATE.items.length}</span>`;
    card.appendChild(wrap);
    ...
    wrap.querySelector('.p024-prev')?.addEventListener('click',event=>{event.preventDefault();event.stopPropagation();move(-1);});
    wrap.querySelector('.p024-next')?.addEventListener('click',event=>{event.preventDefault();event.stopPropagation();move(1);});
    wrap.addEventListener('click',event=>{if(!event.target.closest('.p024-nav'))openFlyer(item);});
    wrap.addEventListener('touchstart',event=>{
      const t=event.touches?.[0];if(!t)return;STATE.touchX=t.clientX;STATE.touchY=t.clientY;
    },{passive:true});
    wrap.addEventListener('touchend',event=>{
      const t=event.changedTouches?.[0];if(!t)return;
      const dx=t.clientX-STATE.touchX,dy=t.clientY-STATE.touchY;
      if(Math.abs(dx)>42&&Math.abs(dx)>Math.abs(dy)){event.preventDefault?.();move(dx<0?1:-1);}
    },{passive:false});
  }
  ...
  function startAuto(card,item){
    ...
    STATE.timer=setInterval(()=>{
      const current=Array.isArray(v51TodayItems)?v51TodayItems[v51TodayIndex]:null;
      if(!current||String(current.weekly_flyer_id)!==String(STATE.flyerId)){stop();return;}
  ...
  v51PaintToday=async function(){
    ...
    const products=normalizedItems(flyer);
  ...
  window.addEventListener('resize',()=>{
    const card=document.getElementById('v37BriefCard');
    if(card?.classList.contains('p024-flyer-card'))syncHeight(card);
  });
