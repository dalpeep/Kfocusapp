...
function normalizeBoardType(v=''){
  ...
  if(['rent','rental','렌트','임대','하우징','sale','매매','realestate','부동산'].includes(s)) return 'realestate';
  return 'notice';
}
function boardLabel(type){
  return ({ notice:'행사안내', news:'뉴스·칼럼', job:'구인구직', realestate:'부동산' })[normalizeBoardType(type)] || '행사안내';
}
function boardThumbEmoji(type){
  return ({ notice:'🎉', news:'📰', job:'💼', realestate:'🏠' })[normalizeBoardType(type)] || '📝';
}
...
function boardListItemHTML(post){
  ...
  const image = boardImages(post)[0] || '';
  const hasVideo = !!String(post.video_url || '').trim();
  const meta = [post.author_name, post.created_at ? String(post.created_at).slice(0,10) : ''].filter(Boolean).join(' · ');

  return `
    <button class="board-card-v3" data-board-post="${esc(post.id)}" data-post-id="${esc(post.id)}">
      <span class="board-card-media">
        ${image ? `<img src="${esc(image)}" alt="${esc(post.title || '')}" loading="lazy">` : `<span class="board-card-fallback">${boardThumbEmoji(type)}</span>`}
        <span class="board-card-type">${esc(boardLabel(type))}</span>
        ${hasVideo ? '<span class="board-card-video">▶ 영상</span>' : ''}
      </span>
    ...
    page.innerHTML = `
      ...
      <article class="board-detail-v3">
        <div class="board-detail-v3-head">
          <div class="board-detail-v3-badges"><span>${esc(boardLabel(normalizedType))}</span>${post.video_url?'<span class="video">▶ 영상</span>':''}</div>
