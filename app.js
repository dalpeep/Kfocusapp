...
function v38Context(){
  ...
  return {now,weekend,events,life,coupons,picks,featured};
}
...
function v38Candidates(ctx){
  ...
  ctx.picks.forEach(r=>rows.push({kind:'dalpick',id:String(r.id),title:r.title||'오늘의 추천',summary:v38Text(r.recommendation_reason||r.summary||r.content||'오늘의 상황에 어울리는 추천입니다.'),data:r,score:70+v38AgeScore(r)+v38KeywordBoost(r.title,ctx)}));
...
function v37RecommendationTags(item){
  ...
  add(`AI ${Math.round(item?.score||80)}점`);
...
function paintV37Recommendation(){
  ...
  if(!title||!summary||!dots)return; const item=v37RecommendationItems[v37RecommendationIndex]||{title:'오늘 가볼 만한 곳',summary:'달라스의 새로운 추천을 확인해 보세요.',score:80};
...
function v38LifeBrief(ctx,candidates){
  ...
  else if(signals.heat){summary='오늘 달라스는 무더운 날씨가 예상됩니다. 외출을 계획했다면 오전이나 저녁 시간이 조금 더 편할 수 있고, 시원한 음식이나 실내 공간도 좋은 선택이 될 수 있습니다.';tip='차량 내부 온도가 빠르게 올라가므로 어린이와 반려동물을 차 안에 두지 마세요.';checklist=['물병','선크림'];pattern='폭염 생활';}
  else if(signals.traffic){summary='오늘 도로 통제나 교통 관련 안내가 확인됩니다. 평소보다 조금 일찍 출발하거나 주요 교차로와 행사장 주변의 우회 경로를 미리 살펴보는 것도 도움이 될 수 있습니다.';tip='출발 직전 지도 앱에서 실시간 교통 상황을 확인하면 불필요한 지연을 줄일 수 있습니다.';checklist=['우회 경로','출발 시간'];pattern='교통 주의';}
  else if(signals.sports){summary='오늘 지역 스포츠 경기 소식이 있습니다. 경기장 주변은 시작 전부터 혼잡할 수 있으니 주차와 이동 시간을 조금 넉넉히 잡아두는 편이 좋겠습니다.';tip='경기 전후 주변 식당이 붐빌 수 있으므로 식사 시간을 조금 앞당기는 것이 좋습니다.';checklist=['티켓','주차'];pattern='스포츠 데이';}
  ...
  else if(signals.sale){summary='오늘 마트 세일과 할인 정보가 확인됩니다. 필요한 식료품이 있다면 장보기 일정을 생각해 보고, 세일 품목도 함께 살펴보면 도움이 될 수 있습니다.';tip='할인 품목은 매장별 재고가 다를 수 있으므로 방문 전 확인하면 좋습니다.';checklist=['장보기 목록','세일 품목'];pattern='마트 세일';}
  else if(signals.weekend){summary='오늘은 주말입니다. 가까운 행사나 가족 나들이 정보를 확인하고, 이동 동선 주변의 맛집과 카페도 함께 둘러보세요.';tip='오후에는 주요 쇼핑몰과 행사장이 혼잡할 수 있으니 오전 시간을 고려해 보는 것도 좋겠습니다.';checklist=['행사','맛집'];pattern='주말 나들이';}
...
function v38FallbackPayload(ctx,candidates){
  ...
  return {title:`오늘의 ${region}`,kicker:`${brief.pattern} · 생활 제안`,summary:brief.summary,tip:brief.tip,checklist:brief.checklist,recommendations:candidates,life:[
    {icon:brief.signals.rain?'🛒':'🎟️',title:brief.signals.rain?'미리 장보기':'오늘 쿠폰',subtitle:ctx.coupons.length?`${ctx.coupons.length}개 혜택`:'혜택 보기'},
    {icon:brief.signals.performance?'🎭':(ctx.weekend?'🎪':'🎉'),title:brief.signals.performance?'오늘 공연':(ctx.weekend?'주말 행사':'오늘 행사'),subtitle:ctx.events.length?`${ctx.events.length}건 새 소식`:'행사 확인'},
    ...
    {icon:brief.signals.sale?'🛒':'📰',title:brief.signals.sale?'마트 세일':'달라스 라이프',subtitle:ctx.life.length?`${ctx.life.length}건 읽기`:'생활 정보 보기'}
  ],source:'생활 패턴 자동 분석'};
}
